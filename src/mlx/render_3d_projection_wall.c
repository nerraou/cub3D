...
void	render_3d_projection_wall(t_data *data, float angle_ray, int ray_num)
{
	...
	distance_to_projection = ((data->width / 2)
			/ tan(data->map->ray->fov_angle / 2));
	ray_distance = cast_ray(data->map, angle_ray, &horizontal, &ray_hit_point);
	ray_distance = ray_distance * \
	cos(angle_ray - data->map->player.rotation_angle);
	wall_height = data->map->scale / ray_distance * \
	...
	if (data->map->is_door == 1)
	{
		if (ray_distance < 20)
			...
			draw_door(data, &(data->map->textures.closed_door),
				(t_tuplef){wall_height, ray_hit_point}, ray_num);
	}
	else
		apply_wall_texture(data, define_texture(data->map, angle_ray, \
		horizontal), (t_tuplef){wall_height, ray_hit_point}, ray_num);
}
