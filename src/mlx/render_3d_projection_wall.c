...
/*   Updated: 2022/11/21 10:44:22 by nerraou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_3d_projection_wall(t_data *data, float angle_ray, int ray_num)
{
	...
	if (data->map->is_door == 1 && horizontal)
		draw_open_door(data, &(data->map->textures.door), (t_tuplef){wall_strip_height, ray_hit_point}, ray_num);
	else
	{
